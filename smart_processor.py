#!/usr/bin/env python3
"""
GNSS.AI Smart Processor v3.2
- Procesa NMEA desde K222/K902/K922
- Envía TODO por FIFO (/tmp/gnssai_smart) para Bluetooth
- Actualiza JSON para dashboard
- Integra ML (si gnss_ml_classifier está disponible)
- Esqueleto para TILT (pitch/roll/heading) listo para K222
"""
...
# ML opcional
try:
    from gnss_ml_classifier import GNSS_ML_Classifier
    ML_AVAILABLE = True
except ImportError:
    ML_AVAILABLE = False
...
class SmartProcessor:
    def __init__(self):
        ...
        if self.ml_enabled:
            try:
                ...
                self.classifier = GNSS_ML_Classifier(model="hybrid")
                print("   ✅ ML listo.")
    ...
    def setup_fifo(self):
        ...
        # Intentar ajustar permisos (si tenemos permiso)
        try:
        ...
        # Intentar abrir para escritura (si ya hay lector)
    ...
    def write_output(self, data: str):
        ...
        if self.fifo_fd is not None:
            try:
                ...
                if e.errno in (errno.EPIPE, errno.ENXIO):
                    # lector se desconectó; cerramos y volveremos a intentar
        ...
        # 4) Actualizar JSON cada 20 frases (por ejemplo)
    ...
    def update_dashboard_json(self):
        ...
        try:
            ...
            # No rompemos el loop por un error de disco
    ...
    def parse_tilt_sentence(self, line: str):
        ...
        PASOS PRÁCTICOS:
        1. En el Pi, ejecuta:   sudo cat /dev/serial0 | grep -i 'sti'   (o 'tilt' / 'ins')
        2. Identifica la sentencia que lleve roll/pitch/heading.
        3. Ajusta el parsing abajo.
        """
        # ⚠️ DE MOMENTO ESTO ES SOLO UN EJEMPLO
        if not line.startswith("$"):
            return

        # EJEMPLO: $PSTI,030,roll,pitch,heading,tilt_flag,...
    ...
    def process_nmea_line(self, line: str):
        ...
        # ML: alimentar con GSV
        if self.ml_enabled and self.classifier and "GSV" in line:
            try:
        ...
        # TILT: llamar al parser de tilt
        self.parse_tilt_sentence(line)

        # Escribir salida NMEA
        self.write_output(line + "\r\n")

    def run(self):
        print("============================================================")
        print("🛰️  GNSS.AI Smart Processor v3.2 (con TILT esqueleto)")
        print("============================================================")
